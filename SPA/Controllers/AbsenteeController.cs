        ...
        public async Task<IActionResult> DeleteAbsentee(int ProjectId, string WhichDatabase)
        {
            ...
            try
            {
                if (WhichDatabase == "Local")
                {
                    ...
                    if (deleteabsentee.Count == 0)
                    {
                        return BadRequest("No absentee found for this ProjectId");
                    }
                    else
                    {
                        ...
                        foreach (var del in deleteabsentee)
                        {
                            var userIdClaim = HttpContext.User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Name);
                            if (userIdClaim != null && int.TryParse(userIdClaim.Value, out int userId))
                            {
                                _logger.LogEvent($"Absentee Entry Deleted for {del.RollNo} ", "Absentee", userId, WhichDatabase);
                            }
                        }
